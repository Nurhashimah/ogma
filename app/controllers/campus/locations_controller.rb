class Campus::LocationsController < ApplicationController
  ...
  def kewpa7
    ...
    @asset_placements=AssetPlacement.where('id IN(?)', AssetPlacement.where(location_id: params[:id]).pluck(:id).uniq)    
...
#     @asset_placements = AssetPlacement.where('id IN(?)', pp)
  ...
  private
    ...
    def location_params
      params.require(:location).permit(:repairdate, :code, :name, :parent_id, :lclass, :typename, :allocatable, :occupied, :staffadmin_id, :staff_name, :ancestry, :parent_code, :college_id, {:data => []}, damages_attributes: [:id, :description,:reported_on,:document_id, :repaired_on])
    end
end