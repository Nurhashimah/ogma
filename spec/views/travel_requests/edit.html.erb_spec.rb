...
RSpec.describe "staff/travel_requests/edit", :type => :view do
  before(:each) do
    ...
    @travel_request = FactoryGirl.create(:travel_request, is_submitted: false, destination: 'some destination', depart_at: DateTime.now-2.days, return_at: DateTime.now-1.days, applicant: @admin_user.userable, replacement: @replacement, headofdept: @hod, destination: "Putrajaya", dept_car: true) 
  ...
  it "renders the edit travel_request form" do
    ...
    assert_select "h1", :text => "#{I18n.t('staff.travel_request.edit')}"
    
    #the FORM starts here
    assert_select "form[action=?][method=?]", staff_travel_request_path(@travel_request), "post" do
      ...
      assert_select "a[href=?]", staff_travel_request_path(@travel_request), {text: I18n.t("helpers.links.back")}
#       also works
