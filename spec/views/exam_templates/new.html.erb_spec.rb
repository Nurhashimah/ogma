...
RSpec.describe "exam/exam_templates/new", :type => :view do
  before(:each) do
    @staff_user=FactoryGirl.create(:staff_user)
    sign_in(@staff_user)
    @exam_template=FactoryGirl.build(:exam_template)
  end

  it "renders new exam_template form" do
    render

    assert_select "form[action=?][method=?]", exam_exam_templates_path, "post" do
...
#       assert_select "input#exam_template__question_count_mcq[count][name=?]", "exam_template[question_count][mcq[count]]"
#       
