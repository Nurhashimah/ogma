...
RSpec.describe "staff/average_instructors/index", :type => :view do
  ...
  before(:each) do
    ...
    sign_in(@admin_user)
    @instructor1=FactoryGirl.create(:basic_staff_with_rank)
    @evaluator1=FactoryGirl.create(:basic_staff_with_rank)
    @instructor2=FactoryGirl.create(:basic_staff_with_rank)
    @evaluator2=FactoryGirl.create(:basic_staff_with_rank)
    @average_instructor1=FactoryGirl.create(:average_instructor, instructor_id: @instructor1.id, evaluator_id: @evaluator1.id)
    @average_instructor2=FactoryGirl.create(:average_instructor, instructor_id: @instructor2.id, evaluator_id: @evaluator2.id)
    @search=AverageInstructor.search(params[:q])
    @average_instructors=@search.result.page(params[:page]).per(5)
...
#     ])
  ...
  it "renders a list of average_instructors" do
    ...
    assert_select "tr>td>a", :text => "#{@average_instructor1.instructor.staff_with_rank}", :count => 1
    assert_select "tr>td>a", :text => "#{@average_instructor2.instructor.staff_with_rank}", :count => 1
    assert_select "td", :text => "24-11-2017", :count => 2
    assert_select "td", :text => "My Title".to_s, :count => 2
    assert_select "td", :text => "Objective line".to_s, :count => 2
    assert_select "td", :text => "Teori", :count => 2
    assert_select "td", :text => "#{@average_instructor2.evaluator.staff_with_rank}", :count => 1
    assert_select "td", :text => "#{@average_instructor2.evaluator.staff_with_rank}", :count => 1
